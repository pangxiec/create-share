...
import com.example.creation.xo.mapper.CommentMapper;
...
import com.example.creation.base.enums.ECommentSource;
...
public class CommentServiceImpl extends SuperServiceImpl<CommentMapper, Comment> implements CommentService {
    ...
    public IPage<Comment> getPageList(CommentVO commentVO) {
        ...
        if (blogUidSet.size() > 0) {
            blogList = articleService.listByIds(blogUidSet);
        }
        Map<String, Article> blogMap = new HashMap<>();
        blogList.forEach(item -> {
            // 评论管理并不需要查看文章内容，因此将其排除
            item.setContent("");
